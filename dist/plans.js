...
const VALID_STATUSES = new Set(["planned", "in-progress", "done", "blocked", "skipped"]);
...
export async function readPlan(filePath, rootDir, isArchived = false) {
...
export async function loadPlans(plansDir) {
...
export function buildQueueHealth(active, archived) {
    const warnings = [];
    const failures = [];
    ...
    for (const plan of active) {
        if (plan.metadata.status !== "planned" && plan.metadata.status !== undefined)
            continue;
...
export function selectPlan(health, item) {
    if (!item) {
...
export async function normalizePlanIfNeeded(plan, now) {
    if (plan.hasMetadata) {
        return plan;
    }
    const metadata = {
...
export async function updatePlanStatus(plan, status, now, reason) {
...
export async function archivePlan(plan, plansDir, now) {
    const root = path.resolve(plansDir);
