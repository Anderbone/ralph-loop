...
import { archivePlan, buildQueueHealth, loadPlans, normalizePlanIfNeeded, readPlan, selectPlan, updatePlanStatus } from "./plans.js";
...
export async function run(options) {
    ...
    if (plansRoot)
        await assertDirectory(plansRoot, "plans");
    const warnings = [];
    const failures = [];
    const records = [];
    ...
    if (options.planFiles.length > 0) {
        ...
        if (failures.length > 0)
            throw new Error(`Ralph finished with ${failures.length} failure(s).`);
        return;
    }
    if (!plansRoot) {
    ...
    if (options.dryRun) {
        ...
        if (health.failures.length > 0)
            throw new Error("Queue health has failures.");
        return;
    }
    let processed = 0;
    while (processed < options.maxItems) {
        ...
        if (record.simplifyStatus === "failed")
            failures.push(`${record.plan}: simplify failed - ${record.simplifyMessage ?? ""}`);
        processed += 1;
        if (options.once || options.item)
            break;
    }
...
async function runExplicitPlans(options, records, warnings, failures, startedAt, summaryPath) {
...
async function runOnePlan(options, plan, plansRoot) {
    ...
    current = await updatePlanStatus(current, implementation.result, new Date().toISOString(), implementation.result === "blocked" ? implementation.message : undefined);
    if (implementation.result === "blocked") {
        return { plan: current.path, status: "blocked", message: implementation.message ?? "Blocked." };
    }
    let simplifyStatus = "skipped";
...
async function finishSummary(options, startedAt, summaryPath, records, warnings, failures) {
...
export function parseArgs(args) {
    const options = {
        repo: "",
        planFiles: [],
        provider: process.env.RALPH_PROVIDER ?? "codex",
    ...
    if (options.provider !== "codex")
        throw new Error("Only --provider codex is implemented in this version.");
    if (options.planFiles.length > 0 && options.plansDir)
        throw new Error("Use either --plans or repeated --plan, not both.");
    if (options.item && !options.plansDir)
        throw new Error("--item requires --plans <dir>.");
...
function parseModelTier(value) {
    if (value === "low" || value === "medium" || value === "high")
        return value;
    throw new Error(`Invalid model tier: ${value}`);
}
function parseCodexLevel(value) {
...
async function getDirtyWarning(repo) {
    try {
